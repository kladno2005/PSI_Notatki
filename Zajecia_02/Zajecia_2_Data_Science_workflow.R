...
colnames(kraje_2) = c("Kod_kraju", "Nazwa", "Region", "Urbanizacja_proc.",
                      "Internet_proc.")
...
# 6.1 mutate() – tworzenie nowych zmiennych na bazie istniejących ----
...
# 6.2 filter() – wybieranie wierszy ----
# 6.3 select() – wybieranie kolumn ----
...
# kraje_reg = kraje[kraje$Region == "Sub-Saharan Africa", c("Panstwo", "PKB_per_capita",
#                                                                     "Populacja_mln", "Urbanizacja_proc.")]


# # Krok 2: Sortowanie
# kraje_reg[order(kraje_reg$PKB_per_capita, decreasing = TRUE), ]

# 6.5 group_by() – grupowanie ----
# 7 summarise() - obliczanie wartości zagregowanych (np. średnich, sum) ----
...
# Dla każdego regionu świata: oblicz liczbę krajów (n), średni % dostęp do internetu i
# średni % poziom urbanizacji, a następnie posortuj regiony malejąco wg średniego %
...
# ale w wielu przykładach użycie funkcji z pakietu dplyr jest bardziej czytelne i
# szybsze.
...
# 8.1 Prosty wykres punktowy: urbanizacja a PKB per capita ----
...
# 8.2 Zaawansowany wykres punktowy: urbanizacja a PKB per capita ----
...
# 8.3 Zaawansowany wykres punktowy: rozmiar gospodarki a populacja ----
ggplot(kraje, aes(x = Populacja_mln, y = PKB, size = PKB_per_capita, color =
                    Region)) +
...
# 8.4 Prosty wykres słupkowy: liczba krajów w regionach ----
...
# 8.5 Zaawansowany wykres słupkowy poziomy: TOP 15 najbogatszych krajów ----
kraje %>%
  arrange(desc(PKB_per_capita)) %>%
  head(15) %>%
  ggplot(aes(x = reorder(Panstwo, PKB_per_capita), y = PKB_per_capita, fill =
               Region)) +
  geom_col() +
  coord_flip() +
...
# 8.6 Wykres pudełkowy (boxplot): dostęp do internetu według regionów ----
...
# 8.7 Wykres pudełkowy (boxplot): przyrost populacji według regionów ----
